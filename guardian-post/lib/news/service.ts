...
export async function getNewsList(): Promise<NewsItem[]> {
  try {
    ...
    // 2. 글로벌(영문) 뉴스 수집 - Restored & Auto-Translated
    ...
    if (sortedNews.length > 0) {
      ...
      // [Auto-Translation] Apply AI Titles to List View
      // Since we just analyzed everything, the cache is populated.
      // We overwrite the original English title/summary with the Korean version generated by AI.
      sortedNews.forEach(item => {
        const cached = aiJournalist.getCachedResult(item.id);
        if (cached && cached.title) {
          item.title = cached.title; // Replace English Title with Korean AI Title
